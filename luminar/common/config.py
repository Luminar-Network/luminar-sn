...
class Settings:
    ...
    netuid: int = field(default_factory=lambda: _env_int("LUMINAR_NETUID", 87))

    # Backend
    backend_url: str = field(
        default_factory=lambda: _env("LUMINAR_BACKEND_URL", "http://localhost:8000").rstrip("/")
    )
    http_timeout: int = field(default_factory=lambda: _env_int("LUMINAR_HTTP_TIMEOUT", 30))

    # Validator loop
    validator_poll_interval: int = field(
        default_factory=lambda: _env_int("LUMINAR_VALIDATOR_POLL_INTERVAL", 60)
    )
    weight_interval_blocks: int = field(
        default_factory=lambda: _env_int("LUMINAR_WEIGHT_INTERVAL_BLOCKS", 100)
    )
    weight_monitor_interval: int = field(
        default_factory=lambda: _env_int("LUMINAR_WEIGHT_MONITOR_INTERVAL", 300)
    )

    # Best-agent cache
    best_agent_dir: Path = field(
        default_factory=lambda: Path(_env("LUMINAR_BEST_AGENT_DIR", ".luminar_cache/best_agent"))
    )
    best_agent_poll_interval: int = field(
        default_factory=lambda: _env_int("LUMINAR_BEST_AGENT_POLL_INTERVAL", 60)
    )

    # Sandbox
    sandbox_image: str = field(
        default_factory=lambda: _env("LUMINAR_SANDBOX_IMAGE", "luminar-sandbox:latest")
    )
    sandbox_workdir: Path = field(
        default_factory=lambda: Path(_env("LUMINAR_SANDBOX_WORKDIR", "/tmp/luminar_sandbox"))
    )

    # Logging
    log_level: str = field(default_factory=lambda: _env("LOG_LEVEL", "INFO"))
    log_dir: Path = field(default_factory=lambda: Path(_env("LUMINAR_LOG_DIR", "logs")))
    log_file_enabled: bool = field(
        default_factory=lambda: _env_bool("LUMINAR_LOG_FILE_ENABLED", True)
    )
    log_filename: Path = field(
        default_factory=lambda: Path(_env("LOG_FILENAME", "luminar_vali.log"))
    )
