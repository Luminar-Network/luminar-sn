"""
Luminar CLI — Submit your agent file to the Luminar subnet.
"""
...
DEFAULT_BACKEND = os.environ.get("LUMINAR_BACKEND_URL", "http://localhost:8000")
...
@click.option(
    ...
    show_default=True,
    envvar="LUMINAR_BACKEND_URL",
    help="Base URL of the Luminar backend.",
)
@click.pass_context
def cli(ctx: click.Context, backend_url: str) -> None:
    """Luminar CLI — manage your Luminar subnet submissions."""
...
def _request_nonce(backend_url: str, hotkey: str) -> dict:
    ...
    try:
        ...
        raise click.ClickException(  # noqa: B904
            ...
            "Is the server running?  Set --backend-url or $LUMINAR_BACKEND_URL."
        )
...
def submit(
    ...
    """Sign and submit an agent file to the Luminar subnet."""
    backend_url: str = ctx.obj["backend_url"]

    click.echo()
    click.echo(click.style("  Luminar — Agent Submission", fg="cyan", bold=True))
...
def status(ctx: click.Context) -> None:
    """Check the status of a past submission."""
    backend_url: str = ctx.obj["backend_url"]

    submission_id = click.prompt(
        click.style("  Submission ID", fg="yellow"),
        default="default",
    )

    try:
        r = requests.get(
            f"{backend_url}/v1/submissions/{submission_id}",
            timeout=15,
        )
    except requests.ConnectionError:
        raise click.ClickException(  # noqa: B904
    ...
    if r.status_code == 200:
        ...
        click.echo()
        click.echo(click.style("  Submission Details", fg="cyan", bold=True))
        click.echo(click.style("  " + "─" * 30, fg="cyan"))
        for key, val in data.items():
            click.echo(f"  {click.style(key + ':', fg='yellow')} {val}")
        click.echo()
    elif r.status_code == 404:
        raise click.ClickException(f"Submission '{submission_id}' not found.")
    else:
